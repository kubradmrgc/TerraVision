...
export default function ProfileChatSessionPage() {
  ...
  useEffect(() => {
    ...
    if (isConsultant) {
      router.replace(`/consultant/chat/${sessionId}`);
      return;
    }
    if (!isCustomer) {
    ...
    void loadChat();
  }, [ready, isAuthenticated, isCustomer, isConsultant, sessionId, router, loadChat]);

  useEffect(() => {
    if (!ready || !isAuthenticated || !Number.isFinite(sessionId)) {
      return;
    }
  ...
  }, [ready, isAuthenticated, sessionId, scrollToBottom]);
  ...
  return (
    <div>
      ...
      <header className="tv-ar-page-header">
        <div>
          <Link href="/profile/chat" className="tv-muted">
            ← Sohbetler
          </Link>
          <h1 className="tv-page-title">{session?.title ?? 'Peyzaj sohbeti'}</h1>
          {session ? (
            <p className="tv-page-lead">
              {session.customerName} · {session.consultantName}
            </p>
          ) : null}
        </div>
      </header>
